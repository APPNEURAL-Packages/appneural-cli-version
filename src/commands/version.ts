...
async function bumpVersion(type: BumpType): Promise<string> {
  try {
    const raw = await fs.readFile(PACKAGE_PATH, "utf-8");
    const pkg = JSON.parse(raw);
    const version: string = pkg.version ?? "0.0.0";
    const segments = version.split(".").map((seg: string) => parseInt(seg, 10) || 0);
    if (type === "patch") {
...
export function registerVersionCommands(program: Command): void {
  ...
  (["patch", "minor", "major"] as BumpType[]).forEach((type) => {
    version
      .command(type)
      .description(`Create an APPNEURAL ${type} release`)
      .action(() =>
        withTelemetry(`version:${type}`, async () => {
